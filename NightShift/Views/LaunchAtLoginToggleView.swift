    ...
    var body: some View {
        Toggle("Launch at Login", isOn: $settings.launchAtLoginEnabled)
            ...
            .onChange(of: settings.launchAtLoginEnabled) { _, newValue in
                settings.launchAtLoginEnabled = LaunchAtLoginService.setEnabled(newValue)
            }
            .onAppear {
                // Deferred to the next runloop turn: writing straight to an
                // observable property from onAppear can land mid-transaction
